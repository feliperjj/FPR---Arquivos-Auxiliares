...
void questao02 (TLista L1, int p, int n, TLista *L2)
{
	...
	TLista aux1, aux2;
	// Começando aux1 e aux2 no começo de suas respectivas listas
	aux1 = L1;
	aux2 = *L2;
	int elementos = 0, posicao=0;
	// Conferindo se o n é valido
	if (n>0){
		// Conferindo se o p é válido
		if (p>=0){
			// Andando com a posição até chegar na posição desejada
			while (posicao != p){
				aux1=aux1->prox;
				posicao++;
			}
			// Andando mais uma posição para não inserir o valor contido na posição desejada pelo usuário e sim, uma adiante.
			aux1=aux1->prox;
			// Enquanto a quantidade de inserções não chegar até a desejada pelo usuário
			while (elementos != n){
				// Realizar a inserção do valor da L1
				inserir (*&L2, aux1->valor);
				// Andar uma posição adiante da L1
				aux1 = aux1->prox;
				// Atualizando a quantidade de elementos inseridos
				elementos++;
				// Caso a lista 1 chegue no valor nulo, parar o while 
				/*
				OBS : Nessa parte eu tentei colocar como condição no while (elementos != n || aux != NULL), o que eu acredito que seria mais correto,
				no entanto, o programa estava travando. Só consegui assim =) 
				*/
				if (aux1 == NULL){
					break;
				}
			...
			// Se p não for válido, a lista 2 é vazia
		 	*L2 = NULL;
		}
	} else {
			// Se n não é válido, a lista 2 é vazia
			*L2 = NULL;
	}
}
